...
def _conv(name, x, filter_size, in_filters, out_filters, strides):
    with tf.compat.v1.variable_scope(name):
        ...
        filter = tf.compat.v1.get_variable('DW', [filter_size, filter_size, in_filters, out_filters], tf.float32,
                                 tf.compat.v1.random_normal_initializer(stddev=WEIGHT_INIT))
        return tf.nn.conv2d(input=x, filters=filter, strides=[1, strides, strides, 1], padding='SAME')
...
def _FC(name, x, out_dim, keep_rate, activation='relu'):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        W = tf.compat.v1.get_variable('DW', [x.get_shape()[1], out_dim],
                            initializer=tf.compat.v1.random_normal_initializer(stddev=WEIGHT_INIT))
        b = tf.compat.v1.get_variable('bias', [out_dim], initializer=tf.compat.v1.constant_initializer())
        ...
        if activation != 'relu':
            ...
            return tf.nn.dropout(x, 1 - (keep_rate))


def _max_pool(x, filter, stride):
    return tf.nn.max_pool2d(input=x, ksize=[1, filter, filter, 1], strides=[1, stride, stride, 1], padding='SAME')
...
def Input():
    ...
    y_ = tf.compat.v1.placeholder(tf.float32, [None, 101])
...
def selective_loss(y_smile_conv, y_gender_conv, y_age_conv, y_, mask):
    ...
    tf.compat.v1.add_to_collection('gender_mask', gender_mask)
    tf.compat.v1.add_to_collection('age_mask', age_mask)
    ...
    tf.compat.v1.add_to_collection('y_gender', y_gender)
    tf.compat.v1.add_to_collection('y_age', y_age)

    smile_cross_entropy = tf.reduce_sum(
        input_tensor=tf.reduce_sum(input_tensor=-y_smile * tf.math.log(y_smile_conv), axis=1) * smile_mask) / tf.clip_by_value(
        tf.reduce_sum(input_tensor=smile_mask), 1, 1e9)
    gender_cross_entropy = tf.reduce_sum(
        input_tensor=tf.reduce_sum(input_tensor=-y_gender * tf.math.log(y_gender_conv), axis=1) * gender_mask) / tf.clip_by_value(
        tf.reduce_sum(input_tensor=gender_mask), 1, 1e9)
    age_cross_entropy = tf.reduce_sum(
        input_tensor=tf.reduce_sum(input_tensor=-y_age * tf.math.log(y_age_conv), axis=1) * age_mask) / tf.clip_by_value(
        tf.reduce_sum(input_tensor=age_mask), 1, 1e9)
...
def train_op(loss, global_step):
    ...
    train_step = tf.compat.v1.train.MomentumOptimizer(learning_rate=learning_rate, momentum=0.9, use_nesterov=True).minimize(loss,
